...
namespace A1DMS.V2
{
    public class GraphicsViewHelper
    {
        ...
        private PropertyItemCollection Properties;
        ...
        public Dictionary<string, object> GetData()
        {
            ...
            data["base"] = this.GetBaseAsync(this.GetBaseQuery(this.Properties));
            data["byCompany"] = this.GetByCompany(this.GetBaseQuery(this.Properties));
            data["byDepartment"] = this.GetByDepartment(this.GetBaseQuery(this.Properties));
            data["byENLDepartment"] = this.GetByDepartment(this.GetBaseQuery(this.Properties).Where(c => c.Company == "ENL"));
        ...
        public void ParseProperties(List<string> sites, List<string> companies, List<string> departments, DateTime? from, DateTime? to)
        {
            ...
            if (sites != null && sites.Count > 0)
                this.Properties.SetValue("Sites", sites);
            else if (siteNames.Count > 0)
                this.Properties.SetValue("Sites", siteNames);

            if (companies != null && companies.Count > 0)
                this.Properties.SetValue("Companies", companies);

            if (departments != null && departments.Count > 0)
                this.Properties.SetValue("Departments", departments);

            if (from.HasValue)
                this.Properties.SetValue("From", from.Value);

            if (to.HasValue)
                this.Properties.SetValue("To", to.Value.AddDays(1).AddSeconds(-1));
        }
