...
export default class Closeout extends TableContainer {
    ...
    showEdit(row) {
        ...
        CommonHelper.fetch("/api/date")
            .then(res => {
            if (res.status == "ok") {
                let date = res.data.split("T")[0];
                CommonHelper.fetch("/api/actionitems/" + row.id)
                    .then(res => {
                    if (res.status == "ok") {
                        this.setState({
                            editFormLoading: false,
    ...
    getColumns() {
        ...
        return [
            ...
            getColumn("Date", "date", true, true, false, row => React.createElement("span", null, CommonHelper.convertDate(row.date))),
            ...
            getColumn("Target Date", "targetDate", true, true, false, row => React.createElement("span", null, CommonHelper.convertDate(row.targetDate)), "closeout-header-cell"),
            ...
            getColumn("Date of Closure", "closureDate", true, true, false, row => React.createElement("span", null, CommonHelper.convertDate(row.closureDate)), "closeout-header-cell")
        ];
    }
}
