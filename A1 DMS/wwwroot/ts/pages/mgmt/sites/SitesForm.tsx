...
export interface SitesFormState {
    redirect: boolean;
    loading: boolean;
}
...
export default class SitesForm extends React.Component<SitesFormProps, SitesFormState> {
    ...
    public render() {
        ...
        return (
            <>
                ...
                <Form<Site> method={this.props.method} url={this.props.url} fields={this.fields} className="form"
                ...
                > 
                    ...
                    <Link className="form-back-link" to="/v2/mgmt/sites">Back to Sites</Link>
                </Form>
            </>
        );
    }

    private handleSuccess(res: FormResponse<Site>) {
        app.notification.show(<div>Site <Link to={"/v2/mgmt/sites/edit/" + res.data.id}>{res.data.name}</Link> successfully {this.props.formType == FormType.new ? "created" : "updated"}</div>);
