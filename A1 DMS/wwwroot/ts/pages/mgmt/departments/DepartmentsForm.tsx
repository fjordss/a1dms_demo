...
export interface DepartmentsFormState {
    redirect: boolean;
    loading: boolean;
}
...
export default class DepartmentsForm extends React.Component<DepartmentsFormProps, DepartmentsFormState> {
    ...
    public render() {
        ...
        return (
            <>
                ...
                <Form<Department> method={this.props.method} url={this.props.url} fields={this.fields} className="form"
                ...
                > 
                    ...
                    <Link className="form-back-link" to="/v2/mgmt/departments">Back to Departments</Link>
                </Form>
            </>
        );
    }

    private handleSuccess(res: FormResponse<Department>) {
        app.notification.show(<div>Department <Link to={"/v2/mgmt/departments/edit/" + res.data.id}>{res.data.name}</Link> successfully {this.props.formType == FormType.new ? "created" : "updated"}</div>);
